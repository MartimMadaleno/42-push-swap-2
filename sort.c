...
{
	int numb = find_media_number(*list_a);
	int op = get_op(*list_a, numb);
	if(op < 0)
	{
		op *= -1;
...
void sort_last_b(t_list **list_a, t_list **list_b)
{
	...
	if (moves_a < 0 &&  moves_b < 0)
	{
		if(moves_a >= moves_b)
		{
			run_function(do_rr, list_a, list_b  , -moves_a);
			tmp_move = moves_a;
			moves_a -= tmp_move;
			moves_b -= tmp_move;
		}
		...
		{
			run_function(do_rr, list_a, list_b  , -moves_b);
			tmp_move = moves_b;
			moves_a -= tmp_move;
			moves_b -= tmp_move;
		}
	...
	else if( moves_a > 0 &&  moves_b > 0)
	{
		if(moves_a <= moves_b)
		{
			run_function(do_rrr, list_a, list_b  , moves_a);
			tmp_move = moves_a;
			moves_a -= tmp_move;
			moves_b -= tmp_move;
		}
		...
		{
			run_function(do_rrr, list_a, list_b  , moves_b);
			tmp_move = moves_b;
			moves_a -= tmp_move;
			moves_b -= tmp_move;
		}
